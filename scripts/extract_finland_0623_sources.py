...
SOURCE_DIR = Path("/Users/jamesm/Downloads/未命名文件夹 2")
OUT_DIR = Path("data/finland_20260623")
TEXT_DIR = OUT_DIR / "pdf_text"

MATCH_ALIASES = {
    "拉赫蒂VSTPS土尔库": ["拉赫蒂VSTPS土尔库"],
    "库普斯VS埃尔维斯": ["库普斯VS埃尔维斯"],
    "瓦萨VSAC奥卢": ["瓦萨VSAC奥卢"],
    "查路VS格尼斯坦": ["查路VS格尼斯坦"],
    "国际图尔库VS塞那乔恩": ["国际图尔库VS塞那乔恩"],
    "玛丽港VS赫尔辛基": ["玛丽港VS赫尔辛基"],
}
...
def detect_match(path: Path) -> str:
    for match, aliases in MATCH_ALIASES.items():
        if any(alias in path.name for alias in aliases):
            return match
    return "主表/其他" if path.name.startswith("【竞彩足球混合】") else ""
...
def main() -> None:
    ...
    for pdf in sorted(SOURCE_DIR.glob("*.pdf")):
        ...
        try:
            ...
            summary["pdfs"][pdf.name] = {
                "match": match, "text_path": str(text_path),
                "mtime": datetime.fromtimestamp(pdf.stat().st_mtime).astimezone().isoformat(),
                **result, "sample": normalize(text_path.read_text(encoding="utf-8"))[:1800],
            }
    ...
    for xls in sorted(SOURCE_DIR.glob("*.xls")):
        ...
        try:
            summary["xls"][xls.name] = {
                "match": match, "kind": classify_xls(xls.name),
    ...
    print(json.dumps({
        "output": str(output), "pdfs": len(summary["pdfs"]), "xls": len(summary["xls"]),
        "matches": {k: {"pdfs": len(v["pdfs"]), "xls": len(v["xls"])} for k, v in summary["matches"].items()},
