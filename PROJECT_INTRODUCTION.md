...
- 使用Elo、Poisson/Dixon-Coles和历史先验生成独立模型概率。
- 用贝叶斯方法融合模型与不同市场证据。
...
- 赔率、盘口和赔率变化优先；Elo/Poisson 只作为独立先验与偏差校验。
...
- 使用近期进失球、主客场拆分和历史先验构造基础 λ。
- ClubElo 或国家队 Elo 只做温和强弱修正，不直接覆盖赔率。
- 赔率总球、三向去水概率和比分盘用于校准 λ 的总量与主客分配。
...
- L：Line，市场与让球深度。
- E：Expected Goals，赔率总球、泊松λ、比分盘和总球空间。
...
| 组件 | 基准权重 | 作用 |
|---|---:|---|
| 当前基础模型 | 55% | 当前阵容、状态、Elo/Poisson和比赛语境 |
| 去水市场模型 | 25% | 市场集体信息校准 |
...
```text
独立预期进球 = 赔率λ × Wmarket + 历史/Elo模型 × (1-Wmarket)
最终预期进球 = 独立预期进球 + 比赛语境修正
```

`Wmarket`不是全局常数。赔率越新、盘口越完整，市场λ权重越高；数据越稀疏、市场分歧越大，历史/Elo先验承担更多稳定作用。
...
- 国际赛样本和轮换信息通常不足，Elo/Poisson 先验仍有估算误差。
